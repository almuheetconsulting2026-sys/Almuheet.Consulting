...
async function doLogin() {
  ...
  let loginName        = ROLE_LABELS[currentRole];
  let loginPermissions = ROLE_PERMISSIONS[currentRole] || [];

  // — تحقق سحابي (اختياري) —
  if (!valid && cloudReady && cloudAuthReady) {
    const cloudUser = await fetchCloudUserByRolePassword(currentRole, passRaw);
    if (cloudUser) {
      valid            = true;
...
async function upsertCloudRoleUser(role, passwordHash) {
  if (!cloudDb || !cloudAuthReady) return;
  const defaults = {
    admin:      ['contracts.edit','contracts.delete','settings.passwords','settings.reset','audit.export','visits.add'],
    engineer:   ['contracts.edit','visits.add'],
    accountant: ['audit.export']
  ...
  try {
    await cloudDb.collection('users').doc(role).set({
      role, name: names[role], password: passwordHash,
...
function queueCloudSave() {
  if (!cloudReady || !cloudDb || !cloudAuthReady || !cloudPullDone) return;
